    ...
    with open(drawable_file) as file:
        ...
        for drawable in drawables_to_update:
            ...
            if os.path.exists(drawable_path):
                ...
                if count > 1:
                    suffix += "_" + n2w(count)
                renamed_drawables.append({"drawable": drawable + suffix,
                                          "name": soup.find(
                                              "item",
                                              attrs={"drawable": drawable})
                                          ["name"] + " Alt " + str(count + 1), })
                os.rename(
                    drawable_path, drawable_path.replace(
                        ".png", suffix + ".png"))
